...
class TelaLerLivros(QMainWindow,Ui_TelaLerDescricao):
    def __init__(self,parent = None):
        super().__init__(parent)   
        self.ui = Ui_TelaLerDescricao()
        self.ui.setupUi(self)         
        edtidlivro = self.ui.lineEdit
        edtidlivro.setValidator(QIntValidator())
        edtidlivro.setMaxLength(4)
        self.ui.btnBuscar.clicked.connect(self.buscar_descricao)
        
    def buscar_descricao(self):
        cursor = conexao.cursor()
        idlivro = self.ui.lineEdit.text()
        cursor.execute(f"SELECT  livro,autor,descricao FROM livros WHERE idlivros ='{idlivro}' and inativo =FALSE")
        for linha in cursor.fetchall():
            print(f"{linha[0]} escrito pelo(a) {linha[1]}")
            self.lineEdit = QLineEdit(self)
            self.lineEdit.setReadOnly(False)
            self.lineEdit.setText(f"{linha[0]} escrito pelo(a) {linha[1]}")
...
class TelaInicial(QMainWindow,Ui_TelaPrinciapal) :
    def __init__(self,parent = None):
        ...
        self.ui.btnlerdescricao.clicked.connect(self.ler_descriacao)
      
    def ler_descriacao(self):
        self.tela = TelaLerLivros()
        self.tela.show()
